...
**Current status: ✅ pins ON and live.** `demo_pins.json` is committed (28 reference hashes baked in: snail1 Female/Pregnant ×8, snail2 Male/Not Pregnant ×10, snail3 Female/Not Pregnant ×10, thresholds crop≤10 / full≤20). Verified locally before pushing: every reference matches its own pin at distance 0, and cross-snail crop distances are ≥18 vs the 10 threshold — the wrong pin can never fire. Live check: `https://snail-api-booth.onrender.com/health` → `"demoPins": 3`. To re-pin after swapping reference photos: edit `SNAILS` in `build_demo_pins.py` → `python build_demo_pins.py` → commit `demo_pins.json` → push. `GEMINI_API_KEY` on `snail-api-booth` is **not set yet** — add it in Render's dashboard so non-pinned scans get real sex/pregnancy instead of a mock result. Full walkthrough in **`BOOTH_PIN_GUIDE.md`**.
